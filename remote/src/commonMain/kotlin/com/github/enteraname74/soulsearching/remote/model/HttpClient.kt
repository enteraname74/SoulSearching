...
sealed interface RemoteResult<T> {
    data class Success<T>(val result: T) : RemoteResult<T> {
        override fun toSoulResult(): SoulResult = SoulResult.Success(result)
    }
    data class Error<T>(val message: String?) : RemoteResult<T> {
        override fun toSoulResult(): SoulResult = SoulResult.Error(message)
    }

    fun toSoulResult(): SoulResult
}
