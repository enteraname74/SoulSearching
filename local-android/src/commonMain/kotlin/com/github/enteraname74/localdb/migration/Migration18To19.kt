...
class Migration18To19(
     ...
     */
    private fun musicMigration(connection: SQLiteConnection): List<CachedMusicUpdate> {
        ...
        connection.execSQL("ALTER TABLE RoomMusic ADD COLUMN albumArtist TEXT")

        val cursor = connection.prepare("SELECT musicId, album, albumId, path FROM RoomMusic")
        ...
        cursor.use {
            while (cursor.step()) {
                val path: String = cursor.getText(0)
    ...
    private fun getMusicArtists(
        ...
        cachedMusicUpdates.chunked(CHUNK_SIZE).forEach { chunk ->
            ...
            val cursor = connection.prepare("SELECT musicId, artistId FROM RoomMusicArtist WHERE musicId IN ($ids)")
            cursor.use {
                while (cursor.step()) {
    ...
    private fun getAlbum(
        ...
        val cursor =
            connection.prepare("SELECT * FROM RoomAlbum WHERE artistId = ${artistId.toSQLId()} AND albumName = ${name.toSQLValue()}")
        cursor.use {
            while (cursor.step()) {
    ...
    private fun getAlbums(
        ...
        albumIds.chunked(CHUNK_SIZE).forEach { chunk ->
            ...
            val cursor = connection.prepare("SELECT * FROM RoomAlbum WHERE albumId IN ($ids)")
            cursor.use {
                while (cursor.step()) {
    ...
    private fun SQLiteStatement.toRoomAlbum(): RoomAlbum? =
        try {
            RoomAlbum(
    ...
    /**
     * Retrieves a mapping of an AlbumArtist name and its id in the connection.
     */
    private fun getAlbumArtistsIds(
        ...
        artistNames.chunked(CHUNK_SIZE).forEach { chunk ->
            ...
            val cursor = connection.prepare("SELECT artistId, artistName FROM RoomArtist WHERE artistName IN ($names)")
            cursor.use {
                while (cursor.step()) {
                    val artistId: SqlUuid = cursor.getId("artistId")
    ...
    private fun updateMusicArtists(
        ...
        newArtistsToSave.chunked(CHUNK_SIZE).forEach { chunk ->
            ...
            connection.execSQL(insertStatement.toString())
        }
    ...
    private fun musicMigrationAndUpdateData(connection: SQLiteConnection) {
        val musicsWithAlbumArtist: List<CachedMusicUpdate> = musicMigration(connection)
    ...
    override fun migrate(connection: SQLiteConnection) {
        ...
        musicMigrationAndUpdateData(connection)
    }
