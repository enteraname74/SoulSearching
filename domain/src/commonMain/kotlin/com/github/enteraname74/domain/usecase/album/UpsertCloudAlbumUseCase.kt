...
class UpsertCloudAlbumUseCase(
    ...
    suspend operator fun invoke(cloudAlbum: CloudAlbum): Album {
        val existingAlbum: Album? = getExistingAlbum(cloudAlbum)
        val albumArtist: Artist = upsertCloudArtistUseCase(cloudAlbum.artist)

        val savedAlbum: Album = existingAlbum?.merge(cloudAlbum, albumArtist)
